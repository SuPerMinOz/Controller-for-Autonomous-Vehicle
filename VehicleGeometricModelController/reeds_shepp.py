...
class PATH:
    def __init__(self, lengths, ctypes, L, x, y, yaw, directions):
        ...
        # type of each part of the path [string]
        self.ctypes = ctypes
...
def CCSC(x, y, phi, paths):
    ...
    if flag:
        paths = set_path(paths, [t, -0.5 * math.pi, u,
                         v], ["WB", "R", "S", "WB"])

    flag, t, u, v = LRSL(-x, y, -phi)
    if flag:
        paths = set_path(
            paths, [-t, 0.5 * math.pi, -u, -v], ["WB", "R", "S", "WB"])

    flag, t, u, v = LRSL(x, -y, -phi)
    if flag:
        paths = set_path(
            paths, [t, -0.5 * math.pi, u, v], ["R", "WB", "S", "R"])
    ...
    if flag:
        paths = set_path(
            paths, [t, -0.5 * math.pi, u, v], ["WB", "R", "S", "R"])
    ...
    if flag:
        paths = set_path(
            paths, [-t, 0.5 * math.pi, -u, -v], ["R", "WB", "S", "WB"])
    ...
    if flag:
        paths = set_path(paths, [v, u, -0.5 * math.pi,
                         t], ["WB", "S", "R", "WB"])

    flag, t, u, v = LRSL(-xb, yb, -phi)
    if flag:
        paths = set_path(
            paths, [-v, -u, 0.5 * math.pi, -t], ["WB", "S", "R", "WB"])

    flag, t, u, v = LRSL(xb, -yb, -phi)
    if flag:
        paths = set_path(
            paths, [v, u, -0.5 * math.pi, t], ["R", "S", "WB", "R"])
    ...
    if flag:
        paths = set_path(
            paths, [v, u, -0.5 * math.pi, t], ["R", "S", "R", "WB"])
    ...
    if flag:
        paths = set_path(
            paths, [-v, -u, 0.5 * math.pi, -t], ["WB", "S", "WB", "R"])
...
def CCSCC(x, y, phi, paths):
    ...
    if flag:
        paths = set_path(
            paths, [t, -0.5 * math.pi, u, -0.5 * math.pi, v], ["WB", "R", "S", "WB", "R"])
    ...
    if flag:
        paths = set_path(
            paths, [t, -0.5 * math.pi, u, -0.5 * math.pi, v], ["R", "WB", "S", "R", "WB"])
