...
#include <functional> // To use std::greater<T> useful in picking the minimum weight
using namespace std;

/* Uses Dijkstra's Algorithm to find the Shortest Path 
   that is from an arbitrary vertex to all other vertices
 * Since the following version is implemented using a min-priority queue,
...
vector<int> ShortestPath(numVertices); // Have an array to store the shortest path

void createAndAddEdge(vector<vector<Pair>> &adjList, int u, int weight, int v) {
  adjList[u].push_back(make_pair(weight, v));
}

void DijkstrasAlgorithm(vector<vector<Pair>> &adjList, int source){
  ...
  for (int i = 0; i < numVertices; i++) {
    if (i != source) {
      ShortestPath[i] = 0x3f3f3f; // Initialize everything else to +infinity
		}
	}
...
int main () {
  int source = 0;
  vector<vector<Pair>> adjList(numVertices); // Create an array of vectors that contain pairs of adjacent vertex and weight
